...
class CCSD:
    ...
    def __init__(self, wfn, Cinp = False, CC_CONV=6, CC_MAXITER=50, E_CONV=8):
        self.Ehf = wfn.energy() 
        self.nmo = wfn.nmo()
        ...
        self.Escf = 2*np.einsum('ii->', ah[oa,oa]) + np.einsum('iijj->', 2*Vaaaa[oa,oa,oa,oa]) - np.einsum('ijji->', Vaaaa[oa,oa,oa,oa]) + self.Vnuc
        print('SCF ENERGIES')
        print('From wfn', self.Ehf)
        print('Calculated', self.Escf)
        self.compute()
