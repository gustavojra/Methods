...
class RBCCD:
    ...
    def rotate_u(self):
        X = np.block([
                     [ np.identity(self.ndocc), np.zeros_like(self.T1)],
                     [ self.T1.T, np.identity(self.nvir)]])
    ...
    def rotate_gs(self):
        ...
        C[:,o] = Cocc + np.einsum('ua,ia->ui', Cvir, self.T1)
        C[:,v] = Cvir - np.einsum('ui,ia->ua', Cocc, self.T1)

        C = gram_schmidt(C, self.Sao)
        Sorb = np.einsum('up,vq,uv->pq', C, C, self.Sao)
        print('*'*37)
        print('Rotating orbitals...')
        Sorb = np.einsum('up,vq,uv->pq', C, C, self.Sao)
    ...
    def update_integrals(self):
        ...
        Vchem = np.asarray(self.mints.mo_eri(self.C, self.C, self.C, self.C))
        ...
        self.Escf = 2*np.einsum('ii->', h[o,o]) + 2*np.einsum('ijij->', self.Voooo) - np.einsum('ijji->', self.Voooo) + self.Vnuc
        print('New SCF', self.Escf)
    ...
    def __init__(self, wfn, Cguess='nop', CC_CONV=6, CC_MAXITER=50, E_CONV=8, T1_CONV=5):
        ...
        self.Sao = np.array(self.mints.ao_overlap())

        # Save Options
        self.CC_CONV = CC_CONV
        self.E_CONV = E_CONV
        self.CC_MAXITER = CC_MAXITER
        self.T1_CONV = T1_CONV
    ...
    def compute(self):
        ...
        print('MP2 Energy:   {:<15.10f}'.format(self.Ecc + self.Escf))
        ...
        while abs(dE) > E_LIM or max_rms > rms_LIM:
            ...
            print("Max T1 amplitude:      {:< 15.10f}".format(max_T1))
            print("Time required:         {:< 15.10f}".format(time.time() - t))
            print('='*37)
            ite += 1

        print('CC Energy:   {:<15.10f}'.format(self.Ecc + self.Escf))
